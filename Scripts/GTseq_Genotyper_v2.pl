...
while (<PROBES>) {
	...
	#print "$F_Primer{$info[0]}\n";
	$F_PrimerKey{$F_Primer{$info[0]}} = $info[0];
	################
	#my$revComp_F_primer=substr reverse($info[5]), 0, 14;
	#$revComp_F_primer=~ tr/[ACGT]/[TGCA]/;
	#print "$revComp_F_primer\n";
	#$F_PrimerKey{$revComp_F_primer} = $info[0];
	################
